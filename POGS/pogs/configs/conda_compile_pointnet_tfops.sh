#!/bin/bash
# Make sure the conda environment is activated so that CONDA_PREFIX is defined.

# Replace the system paths with conda environment paths:
CUDA_INCLUDE=" -I${CONDA_PREFIX}/include/"
CUDA_LIB=" -L${CONDA_PREFIX}/lib/"

# Get TensorFlow compile and link flags from the active Python:
...
g++ -std=c++11 tf_interpolate.cpp -o tf_interpolate_so.so -shared -fPIC ${TF_CFLAGS} ${TF_LFLAGS} -O2
